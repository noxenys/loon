// 2026/04/21 Universal version for Quantumult X and Loon
/*
QX:
[rewrite_local]
...
function notify(title, body) {
  if (isQX) return $notify(scriptName, title, body);
...
function runAccount(acc, index, total) {
      ...
      return new Promise(resolve => {
        setTimeout(() => {
          ...
          fetchApi("videoBonus").then(res => {
            const d = parseJsonSafe(res.body);
            if (d && d.retcode === 0) {
  ...
  return fetchApi("queryBalanceAndBonus")
    .then(res => {
      const d = parseJsonSafe(res.body);
      if (d && d.retcode === 0) msgs.push(`💰 余额：${d.result.balance} Coins`);
      else if (d) msgs.push(`⚠️ 查询：${d.retmsg}`);
      else msgs.push("❌ 查询：解析失败");
      return fetchApi("checkIn");
    })
    .then(res => {
      const d = parseJsonSafe(res.body);
      if (d && d.retcode === 0) {
        const hint = (d.result && d.result.bonusHint) || d.retmsg || "";
        msgs.push(`✅ 签到：${String(hint).replace(/\n/g, " ")}`);
      } else if (d) {
...
if (typeof $request !== "undefined" && $request) {
  ...
  const headersMap = normalizeHeaderNameMap(($request && ($request.headers || $request.header)) || {});
  let baseUA = "";
